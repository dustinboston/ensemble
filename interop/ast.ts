...
export type SerializedAst = {
	...
	id: string;
	name: string;
	type?: string | SerializedAst | (string | SerializedAst)[];
	...
	public serialize() {
		...
		if (this.type.length) {
			const type = this.type.map((p) => {
				if (!p.id && !p.type.length && !p.typeParameters.length && !p.parameters.length && p.text) return p.text;
				return p.serialize();
			});

			obj.type = (type.length === 1) ? type[0] : type;
		}
