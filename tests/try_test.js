...
import { ErrorNode, NilNode, NumberNode, StringNode } from "../src/types.js";
...
test(`TRY: Testing try*/catch*`, () => {
    ...
    test(`Evaluating try* without error`, () => {
        // 123
        assertEquals(rep(`(try* 123 (catch* e 456))`, sharedEnv), printString(new NumberNode(123), true));
    });
    test(`Evaluating try* with undefined symbol`, () => {
        const consoleLogSpy = spy(console, 'log');
        assertEquals(rep(`(try* abc (catch* exc (prn "exc is:" exc)))`, sharedEnv), printString(new NilNode(), true));
    ...
    test(`Evaluating try* with function call to undefined symbol`, () => {
        const consoleLogSpy = spy(console, 'log');
        assertEquals(rep(`(try* (abc 1 2) (catch* exc (prn "exc is:" exc)))`, sharedEnv), printString(new NilNode(), true));
        assertSpyCalls(consoleLogSpy, 1);
        assertEquals(consoleLogSpy.calls[0].args[0], `"exc is:" "'abc' not found"`);
        consoleLogSpy.restore();
    });
...
test(`TRY: Make sure error from core can be caught`, () => {
    ...
    test(`TRY: Catching core error`, () => {
        const consoleLogSpy = spy(console, 'log');
        assertEquals(rep(`(try* (nth () 1) (catch* exc (prn "exc is:" exc)))`, sharedEnv), printString(new NilNode(), true));
        assertSpyCalls(consoleLogSpy, 1);
    ...
    test(`TRY: Catching thrown exception`, () => {
        const consoleLogSpy = spy(console, 'log');
        assertEquals(rep(`(try* (throw "my exception") (catch* exc (do (prn "exc:" exc) 7)))`, sharedEnv), printString(new NumberNode(7), true));
        assertSpyCalls(consoleLogSpy, 1);
...
test(`TRY: Test that exception handlers get restored correctly`, () => {
    ...
    test(`Restoring exception handlers`, () => {
        assertEquals(rep(`(try* (do (try* "t1" (catch* e "c1")) (throw "e1")) (catch* e "c2"))`, sharedEnv), printString(new StringNode('c2'), true));
    });
    test(`Testing nested try*/catch*`, () => {
        assertEquals(rep(`(try* (try* (throw "e1") (catch* e (throw "e2"))) (catch* e "c2"))`, sharedEnv), printString(new StringNode('c2'), true));
    });
});
test(`TRY: Test that throw is a function`, () => {
    const sharedEnv = initEnv();
    test(`Throwing inside map`, () => {
        assertEquals(rep(`(try* (map throw (list "my err")) (catch* exc exc))`, sharedEnv), printString(new StringNode('my err'), true));
    });
});
...
test(`TRY: Testing throwing non-strings`, () => {
    ...
    test(`Throwing a list`, () => {
        const consoleLogSpy = spy(console, 'log');
        assertEquals(rep(`(try* (throw (list 1 2 3)) (catch* exc (do (prn "err:" exc) 7)))`, sharedEnv), printString(new NumberNode(7), true));
        assertSpyCalls(consoleLogSpy, 1);
