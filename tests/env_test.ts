...
Deno.test('ENV: Should handle undefined symbol', () => {
	assertThrows(
		() => {
...
Deno.test('ENV: Check that error aborts def!', () => {
...
// Testing let*
Deno.test('ENV: Should define and retrieve z', () => {
	assertEquals(rep('(let* (z 9) z)', testEnv), '9');
});

Deno.test('ENV: Should define and retrieve x', () => {
	assertEquals(rep('(let* (x 9) x)', testEnv), '9');
});

Deno.test('ENV: Should retrieve previous value of x', () => {
	assertEquals(rep('x', testEnv), '4');
});

Deno.test('ENV: Should compute expression with z', () => {
	assertEquals(rep('(let* (z (+ 2 3)) (+ 1 z))', testEnv), '6');
});

Deno.test('ENV: Should compute expression with p and q', () => {
	assertEquals(rep('(let* (p (+ 2 3) q (+ 2 p)) (+ p q))', testEnv), '12');
});

Deno.test('ENV: Should define y with let* and retrieve its value', () => {
	assertEquals(rep('(def! y (let* (z 7) z))', testEnv), '7');
	assertEquals(rep('y', testEnv), '7');
});

// Testing outer environment
Deno.test('ENV: Should define and retrieve a', () => {
	assertEquals(rep('(def! a 4)', testEnv), '4');
});

Deno.test('ENV: Should retrieve value of q', () => {
	assertEquals(rep('(let* (q 9) q)', testEnv), '9');
});

Deno.test('ENV: Should retrieve outer environment value of a', () => {
	assertEquals(rep('(let* (q 9) a)', testEnv), '4');
});

Deno.test('ENV: Should retrieve outer environment value of a with nested let*', () => {
	assertEquals(rep('(let* (z 2) (let* (q 9) a))', testEnv), '4');
});

// Testing let* with vector bindings
Deno.test('ENV: Vector bindings should work', () => {
	assertEquals(rep('(let* [z 9] z)', testEnv), '9');
});

Deno.test('ENV: Should compute expression with vector bindings', () => {
	assertEquals(rep('(let* [p (+ 2 3) q (+ 2 p)] (+ p q))', testEnv), '12');
});

// Testing vector evaluation
Deno.test('ENV: Should evaluate vector with bindings', () => {
	assertEquals(
...
Deno.test('ENV: Last assignment takes priority', () => {
