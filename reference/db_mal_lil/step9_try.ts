/**
 * @file Step 9 implements the try/catch special form, as well as many core
 * functions, including: throw, nil?, true?, false?, symbol, symbol?, keyword,
 * keyword?, vector, vector?, hash-map, map?, assoc, dissoc, get, contains?,
 * keys, vals, sequential?, apply, and map.
 */
...
export function handleTry(
	...
	types.assertTryCatch(ast);
	try {
		return {
			return: evaluate(ast.value[1], appEnv),
			continue: undefined,
		};
	} catch (error) {
		if (!ast.value[2]) {
		...
		const sym = ast.value[2].value[1] as types.SymbolNode;
		const list = ast.value[2].value[2] as types.ListNode;
