...
export function quasiquote(node: types.AstNode): types.AstNode {
  const isQuotableNode =
    node instanceof types.MapNode || node instanceof types.SymbolNode;
...
export function evaluateAst(
  ...
  if (node instanceof types.DomNode) {
    ...
    return new types.DomNode(evaluated);
  }
...
export function evaluate(node: types.AstNode, appEnv: env.Env): types.AstNode {
  for (;;) {
    if (showTrace) console.debug(`eval: ${print(node)}`);

    if (!(node instanceof types.ListNode) || node.value.length === 0) {
    ...
    const symbolValue =
      node.value[0] instanceof types.SymbolNode
        ? node.value[0].value
        : "goto_default_clause";
    ...
    switch (symbolValue) {
      ...
      case "function": // JS anonymous function
      case "=>": // JS anonymous function
      case "fn*": {
        // Return
...
/**
 * Parse arguments into a URL-like MapNode.
 * @see https://developer.mozilla.org/en-US/docs/Web/API/URL
 * @returns a MapNode with the same keys as the URL object, specifically:
 * hash, host, hostname, href, origin, password, pathname, port, protocol,
 * search, username and searchParams. searchParams is a map of pairs.
 * @example
 * ```
 * parseArguments(['./foo.js', '-A', 'apple', 4]);
 * // MapNode { hash, host, hostname, ...searchParams, etc. }
 * ```
 */
export function parseArguments(args: string[]): types.MapNode {
  try {
    const location = new URL(args[0], "file://");
    ...
    while (index < args.length) {
      ...
      if (
        ...
        const key = currentArgument.replace(/^-*/, ":");
        location.searchParams.set(key, nextArgument);
        index += 2;
      } else {
        const key = currentArgument.replace(/^-*/, ":");
    ...
    const urlMap = new Map<string, types.AstNode>();
    urlMap.set(":hash", new types.StringNode(location.hash));
    urlMap.set(":hash", new types.StringNode(location.hash));
    urlMap.set(":host", new types.StringNode(location.host));
    urlMap.set(":hostname", new types.StringNode(location.hostname));
    urlMap.set(":href", new types.StringNode(location.href));
    urlMap.set(":origin", new types.StringNode(location.origin));
    urlMap.set(":password", new types.StringNode(location.password));
    urlMap.set(":pathname", new types.StringNode(location.pathname));
    urlMap.set(":port", new types.StringNode(location.port));
    urlMap.set(":protocol", new types.StringNode(location.protocol));
    urlMap.set(":search", new types.StringNode(location.search));
    urlMap.set(":username", new types.StringNode(location.username));

    const searchMap = new Map<string, types.StringNode>();
    location.searchParams.forEach((value, key) => {
      searchMap.set(key, new types.StringNode(value));
    });

    urlMap.set(":searchParams", new types.MapNode(searchMap));
    return new types.MapNode(urlMap);
  } catch (urlError) {
    return new types.MapNode(new Map(), new types.StringNode(urlError.message));
  }
}
...
export function main(...args: string[]): void {
  ...
  replEnv.set(new types.SymbolNode("location"), parsedArguments);
  if (parsedArguments.value.size) {
    const searchParams = parsedArguments.value.get(":searchParams");
    if (
      searchParams &&
      searchParams instanceof types.MapNode &&
      searchParams.value.get(":trace")
    ) {
      showTrace = true;
    }
  }

  replEnv.set(new types.SymbolNode("*ARGV*"), new types.ListNode(hostEnvArgs));
  replEnv.set(
