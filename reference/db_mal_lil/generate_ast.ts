...
type SerializedClassProperties = Omit<
	ClassProperties,
	...
	private generateConcreteVisitors(
		...
		return baseClass.subClasses
			.map((subClass) => {
				...
				const supportTypes = subClass.supportingTypes
					? subClass.supportingTypes.join('\n') + '\n'
					: '';

				// Assign a default to the value parameter
				const valueDefault = subClass.valueDefault
					? ` = ${subClass.valueDefault}`
					: '';
				...
				return `${supportTypes}
					...
					export class ${className}${classGeneric} extends ${baseName} {
						...
						${this.generateTypeGuards(className, typeGuardType, classGeneric)}
						${staticMethods ? '\n' + staticMethods : ''}
