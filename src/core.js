 ...
 * (pr-str "abc\ndef\nghi") ;=> "\"abc\\ndef\\nghi\""
 */
export function printEscapedString(...args) {
 ...
 * @example (str "abc\ndef\nghi") ;=> "abc\ndef\nghi"
 */
export function printUnescapedString(...args) {
 ...
 * ```
 */
export function printEscapedStringToScreen(...args) {
    const result = args.map((arg) => printer.printString(arg, true)).join(' ');
    console.log(result);
 ...
 * @returns types.Nil
 */
export function printUnescapedStringToScreen(...args) {
    const result = args.map((arg) => printer.printString(arg, false)).join(' ');
    console.log(result);
    return types.createNilNode();
}
 ...
 */
export function readString(...args) {
 ...
 */
export function isListNode(...args) {
    types.assertArgumentCount(args.length, 1);
 ...
 * @example (first (+ 2 4) ;=> +
 */
export function firstNodeInList(...args) {
    types.assertArgumentCount(args.length, 1);
...
export function lastNodeInList(...args) {
    types.assertArgumentCount(args.length, 1);
    if (types.isSequentialNode(args[0]) && args[0].value.length > 0) {
 ...
 */
export function applyToSequence(...args) {
    const count = args.length;
...
export function seq(...args) {
    ...
    if (types.isStringNode(ast) && ast.value.length > 0) {
        return types.createListNode(ast.value.split('').map((char) => types.createStringNode(char)));
    }
    return types.createNilNode();
}
/**
 ...
 */
export function withMeta(...args) {
    types.assertArgumentCount(args.length, 2);
    types.assertMetadataType(args[0]);
    types.assertAstNode(args[1]);
    const copy = types.copy(args[0]);
 ...
 */
export function isSymbolNode(...args) {
    types.assertArgumentCount(args.length, 1);
 ...
 */
export function isKeyword(...args) {
    types.assertArgumentCount(args.length, 1);
 ...
 * @example (fn? (fn* )) ;=> true
 */
export function isFn(...args) {
    types.assertArgumentCount(args.length, 1);
    return types.createBooleanNode(types.isFunctionNode(args[0]) && !args[0].isMacro);
}
/**
 ...
 * @example (macro? ) ;=> true
 */
export function isMacro(...args) {
    types.assertArgumentCount(args.length, 1);
    return types.createBooleanNode(types.isFunctionNode(args[0]) ? args[0].isMacro : false);
}
/**
...
export function assoc(...args) {
    ...
    const dict = types.createMapNode(new Map(args[0].value));
 ...
 */
export function isSequentialNode(...args) {
 ...
 */
export function switchCase(...args) {
    types.assertMinimumArgumentCount(args.length, 2);
