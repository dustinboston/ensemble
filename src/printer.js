 ...
 * @throws If it encounters an unmatched or unrecognized AST node type.
 */
export function printHtml(ast, printReadably = false) {
    if (types.isDomNode(ast)) {
        ...
        if (ast.attributes.size > 0) {
            attributes = ' ' + Array.from(ast.attributes).map(([key, value]) => {
                if (key === 'style')
                    return printInlineCss(value, printReadably);
 ...
 * @throws If it encounters an unmatched or unrecognized AST node type.
 */
export function printJavaScript(ast, printReadably = false) {
    if (types.isDomNode(ast)) {
    ...
    if (types.isStringNode(ast) || types.isKeywordNode(ast) ||
        types.isSymbolNode(ast)) {
        return `"${ast.value}"`;
    }
 ...
 */
export function printCss(ast, printReadably = false) {
    if (types.isStringNode(ast)) {
        if (printReadably) {
    ...
    if (types.isBooleanNode(ast) || types.isAtomNode(ast) ||
...
export function printStyleTag(ast, printReadably = false) {
 ...
 * @returns
 */
export function printInlineCss(ast, printReadably = false) {
