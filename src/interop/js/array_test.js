...
test('arrayFrom - two arguments', () => {
    const vec = types.createVectorNode([types.createNumberNode(1), types.createNumberNode(2)]);
    const fn = types.createFunctionNode((x) => types.createNumberNode(x.value * 2));
...
test('copyWithin - two arguments', () => {
    ...
    assertEquals(result.value.map((n) => n.value), [2, 3, 3]);
});
test('copyWithin - three arguments', () => {
    ...
    assertEquals(result.value.map((n) => n.value), [2, 2, 3]);
});
...
test('every - some false', () => {
    const vec = types.createVectorNode([types.createNumberNode(1), types.createNumberNode(2)]);
    const fn = types.createFunctionNode((_index, v, _vector) => types.createBooleanNode(v.value !== 2));
    const result = array.arrayEvery(vec, fn);
    assertEquals(result, types.createBooleanNode(false));
});
test('every - invalid arguments', () => {
    ...
    assertThrows(() => array.arrayEvery(vec, undefined, num));
});
// MARK: fill
// --------------------------------------------------------------------------------------------------------------------
...
test('filter - some match', () => {
    const vec = types.createVectorNode([types.createNumberNode(1), types.createNumberNode(2)]);
    const fn = types.createFunctionNode((item) => types.createBooleanNode(item.value > 1));
...
test('find - element found', () => {
    const vec = types.createVectorNode([types.createNumberNode(1), types.createNumberNode(2)]);
    const fn = types.createFunctionNode((v) => types.createBooleanNode(v.value === 2));
...
test('arrayFlat - basic functionality', () => {
    ...
    assertEquals(result.value.map((n) => n.value), [1, 2, 3]);
});
test('arrayFlat - deeply nested vectors', () => {
    ...
    const result = array.arrayFlat(vec);
    assertEquals(result.value.map((n) => n.value), [1, 2, 3, 4]);
});
...
test('arrayFlat - vector with non-nested elements', () => {
    const vec = types.createVectorNode([types.createNumberNode(1), types.createNumberNode(2)]);
    const result = array.arrayFlat(vec);
    assertEquals(result.value.map((n) => n.value), [1, 2]);
});
...
test('arrayFlatMap - basic functionality', () => {
    const vec = types.createVectorNode([types.createNumberNode(1), types.createNumberNode(2)]);
    const fn = types.createFunctionNode((v) => types.createVectorNode([v, v]));
    const result = array.arrayFlatMap(vec, fn);
    assertEquals(result.value.map((n) => n.value), [1, 1, 2, 2]);
});
test('arrayFlatMap - nested result', () => {
    const vec = types.createVectorNode([types.createNumberNode(1), types.createNumberNode(2)]);
    // Callback returns a nested vector for each element
    const fn = types.createFunctionNode((v) => types.createVectorNode([v, types.createVectorNode([v])]));
    const result = array.arrayFlatMap(vec, fn);
    assertEquals(result.value.map((n) => n.value), [1, 1, 2, 2]);
});
...
test('arrayMap - with thisArg', () => {
    ...
    const fn = types.createFunctionNode((_x, _index, _thisArg) => types.createNumberNode(2));
...
test('arrayReduce - basic functionality', () => {
    const fn = types.createFunctionNode((acc, x) => types.createNumberNode(acc.value + x.value));
    const vec = types.createVectorNode([types.createNumberNode(1), types.createNumberNode(2)]);
...
test('arrayToReversed - basic functionality', () => {
    ...
    assertEquals(result.value.map((n) => n.value), [2, 1]);
});
...
test('arraySlice - two arguments', () => {
    ...
    assertEquals(result.value.map((n) => n.value), [2]);
});
...
test('arrayToSorted - basic functionality', () => {
    ...
    const fn = types.createFunctionNode((a, b) => types.createNumberNode(a.value - b.value));
...
test('arrayToSpliced - three arguments', () => {
    ...
    assertEquals(result.value.map((n) => n.value), [1]);
});
test('arrayToSpliced - four arguments', () => {
    ...
    assertEquals(result.value.map((n) => n.value), [1, 3]);
});
...
test('arrayReplaceWith - replaces with specified value', () => {
    ...
    assertEquals(result.value.map((n) => n.value), [1, 5, 3]);
});
