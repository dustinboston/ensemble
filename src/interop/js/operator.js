 ...
 * @see types.isEqualTo()
 */
export function notEqualTo(...args) {
...
export function increment(...args) {
    ...
    if (args[1] !== undefined) {
        ...
        if (args[1].value !== 'prefix' && args[1].value !== 'postfix') {
            throw new TypeError(`Invalid affix ${String(args[1].value)}. The affix must be "prefix" or "postfix"`);
        }
        else {
            affix = args[1].value;
        }
...
export function typeOf(...args) {
    ...
    if (obj !== 'bigint' &&
        ...
        obj !== 'symbol' &&
        obj !== 'undefined') {
        throw new Error(`Invalid type: "${args[1].value}". Type must be one of bigint, boolean, function, number, object, string, symbol, or undefined`);
    }
 ...
 * @returns types.BooleanNode
 */
export function instanceOf(...args) {
    types.assertArgumentCount(args.length, 2);
    types.assertAstNode(args[0]);
