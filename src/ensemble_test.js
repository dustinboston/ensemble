import { assertEquals, assertExists, assertInstanceOf, assertThrows, test } from "../tests/test_runner.js";
import { add } from "./core.js";
import { evaluate, evaluateApply, evaluateAst, evaluateDef, evaluateDefMacro, evaluateDo, evaluateFn, evaluateIf, evaluateLet, evaluateQuasiQuote, evaluateQuasiQuoteExpand, evaluateQuote, evaluateTry, initEnv, isMacroCall, macroExpand, quasiQuote, } from "./ensemble.js";
import { Env } from "./env.js";
import { assertDefined, assertFunctionNode, createBooleanNode, createDomNode, createFunctionNode, createListNode, createMapNode, createNilNode, createNumberNode, createStringNode, createSymbolNode, createVectorNode, FunctionNode, isFunctionNode, isNumberNode, } from "./types.js";
...
test('evaluateDefMacro(): should define a macro', () => {
    const result = evaluateDefMacro(createListNode([
        createSymbolNode('defmacro!'),
        createSymbolNode('a'),
        createFunctionNode(() => createNilNode()),
    ]), new Env());
    const fn = result.return;
...
test('evaluateDefMacro(): should throw error on invalid key', () => {
    ...
    assertThrows(() => evaluateDefMacro(ast, env), Error, 'Invalid dictionary key');
});
test('evaluateDefMacro(): should copy metadata', () => {
    const result = evaluateDefMacro(createListNode([
        createSymbolNode('defmacro!'),
        createSymbolNode('a'),
        createFunctionNode(() => createNilNode(), undefined, // closureMetadata
        false, // isMacro
        createStringNode('meta')),
    ]), new Env());
    const fn = result.return;
    assertInstanceOf(fn, FunctionNode);
...
test('macroExpand(): should expand a single-layer macro', () => {
    const env = new Env();
    const macroFunc = createFunctionNode((...args) => createListNode([createSymbolNode('quote'), ...args]));
    ...
    const expanded = macroExpand(ast, env);
    assertEquals(expanded, createListNode([
        createSymbolNode('quote'),
        createSymbolNode('a'),
    ]));
});
test('macroExpand(): should expand nested macros', () => {
    const env = new Env();
    const macroFunc1 = createFunctionNode((...args) => createListNode([createSymbolNode('quote'), ...args]));
    macroFunc1.isMacro = true;
    const macroFunc2 = createFunctionNode((...args) => createListNode([createSymbolNode('a_macro1'), ...args]));
...
test('evaluateAst(): should evaluate DomNodes', () => {
    ...
    assertEquals(result, createDomNode('a', new Map([['href', createStringNode('https://example.com')]])));
});
...
test("evaluate(): should handle 'def!' correctly", () => {
    const env = new Env();
    const result = evaluate(createListNode([
...
test('evaluate(): should handle default case correctly', () => {
    const env = new Env();
    env.set(createSymbolNode('+'), createFunctionNode(() => createNumberNode(3)));
    const result = evaluate(createListNode([
        createSymbolNode('+'),
        createNumberNode(1),
        createNumberNode(2),
...
test('evaluateQuasiQuoteExpand(): returns expanded form', () => {
    ...
    const expectedResult = quasiQuote(createListNode([
        createSymbolNode('unquote'),
        createSymbolNode('a'),
    ]));
...
test('evaluateDefMacro(): defines a macro', () => {
    ...
    const fn = env.get(createSymbolNode('one'));
...
test('evaluateTry(): catches thrown error and handles it', () => {
    ...
    const env = initEnv();
    env.set(createSymbolNode('custom-throw'), createFunctionNode((_args, _env) => {
        throw 'error!';
    }));
    const result = evaluateTry(ast, env);
...
test('evaluateTry(): catches native error and handles it', () => {
    ...
    const env = initEnv();
    env.set(createSymbolNode('throw-native'), createFunctionNode((_args, _env) => {
...
test('evaluateFn(): creates a simple anonymous function', () => {
    ...
    const env = new Env();
    env.set(createSymbolNode('+'), createFunctionNode((...args) => createNumberNode(args[0].value +
        args[1].value)));
...
test('evaluateApply(): applies closure correctly', () => {
    ...
    assertEquals(result.continue?.ast, createListNode([
        createSymbolNode('+'),
        createSymbolNode('a'),
        createNumberNode(2),
    ]));
...
test('initEnv(): initEnv runs repl with predefined functions', () => {
    const env = initEnv();
    const fn = env.get(createSymbolNode('not'));
