...
import * as html from "./interop/html.js";
import * as js from "./interop/js.js";
import * as printer from "./printer.js";
import * as reader from "./reader.js";
import * as types from "./types.js";
export * as core from "./core.js";
export * as env from "./env.js";
export * as html from "./interop/html.js";
export * as js from "./interop/js.js";
 ...
 */
export function assertTryCatch(a) {
 ...
 * @example (let* (z 9) z)
 */
export function assertLet(a) {
    types.assertListNode(a); // (...)
    types.assertArgumentCount(a.value.length, 3); // (1 2 3)
    types.assertSymbolNode(a.value[0]); // (sym 2 3)
    // assertSymWithValue(a.value[0], "let*"); // (let* 2 3)
    const symbolNode = a.value[0];
    if (symbolNode.value !== 'let*' &&
 ...
 */
export function assertQuasiQuoteExpand(a) {
 ...
 */
export function assertQuasiQuote(a) {
 ...
 */
export function assertDefMacro(a) {
...
export function assertFn(a) {
    ...
    // (if (Sym*) 3)
    types.assertSequentialValues(a.value[1].value, types.SymbolNode);
    // (if (Sym*) Ast)
...
export function evaluateAst(node, appEnv) {
    ...
    if (types.isDomNode(node)) {
        ...
        const attributes = Array.from(node.attributes).reduce((map, [key, value]) => map.set(key, evaluate(value, appEnv)), new Map());
 ...
 * @example '(+ 1 2) => (+ 1 2)
 */
export function evaluateQuote(node, _) {
    assertQuote(node);
...
export function evaluateDo(node, appEnv) {
    ...
    let lastResult = types.createNilNode();
...
export function evaluateTry(node, appEnv) {
    ...
    try {
        ...
        const sym = node.value[2].value[1];
        const list = node.value[2].value[2];
        let message;
...
export function evaluateFn(node, appEnv) {
    ...
    const fn = types.createFunctionNode((...args) => {
...
export function evaluateApply(node, appEnv) {
    ...
    if (types.isFunctionNode(fn)) {
        ...
        if (fn.closureMeta) {
            ...
            const fnEnv = new env.Env(fn.closureMeta.env, fn.closureMeta.parameters, args);
...
export function initEnv() {
    ...
    replEnv.set(types.createSymbolNode('eval'), types.createFunctionNode((...args) => {
        types.assertArgumentCount(args.length, 1);
        return evaluate(args[0], replEnv);
    }));
    replEnv.set(types.createSymbolNode('dump'), types.createFunctionNode((..._args) => {
