...
import { createListNode, createNilNode, createNumberNode, createStringNode, createSymbolNode, } from "./types.js";
...
test("new Env(): should bind remaining expressions after '&'", () => {
    ...
    assertEquals(env.value.size, 2);
    assertEquals(env.value, new Map([
...
test("new Env(): should bind to an empty list if there are no exprs after '&'", () => {
    ...
    assertEquals(env.get(createSymbolNode('a')), createStringNode('1'));
    assertEquals(env.get(createSymbolNode('b')), createListNode([]));
});
test('new Env(): should set outer environment', () => {
    const outer = new Env(undefined, [createSymbolNode('x')], [createNumberNode(1)]);
...
test('set(): should set a new key-value pair', () => {
    ...
    assertEquals(env.get(createSymbolNode('x')), createListNode([]));
});
...
test('get(): should throw error when getting non-existent key', () => {
    const env = new Env(undefined);
    assertThrows(() => env.get(createSymbolNode('z')), Error, "'z' not found");
});
test('get(): should find a value in the outer environment', () => {
    const outer = new Env(undefined, [createSymbolNode('x')], [createNumberNode(1)]);
    const inner = new Env(outer);
    assertEquals(inner.get(createSymbolNode('x')), createNumberNode(1));
});
test('get(): should shadow value in inner environment', () => {
    const outerEnv = new Env(undefined, [createSymbolNode('a')], [createNilNode()]);
    const innerEnv = new Env(outerEnv, [createSymbolNode('a')], [createListNode([])]);
