...
import { readAtom, Reader, readForm, readSequence, readString, tokenize, unescapeString } from "./reader.js";
import { assertDefined, createBooleanNode, createKeywordNode, createListNode, createMapNode, createNilNode, createNumberNode, createStringNode, createSymbolNode, createVectorNode, } from "./types.js";
...
test('tokenize(): should tokenize complex nested expressions', () => {
    assertEquals(tokenize('(/ (- (+ 515 (* -87 311)) 296) 27)'), [
        '(',
        '/',
        '(',
        '-',
...
test('tokenize(): should filter out comments', () => {
    assertEquals(tokenize(';; single line\n(+ 1 2)'), ['(', '+', '1', '2', ')']);
});
test('tokenize(): should filter out trailing comments', () => {
    assertEquals(tokenize('(+ 1 2) ;; trailing'), ['(', '+', '1', '2', ')']);
});
...
test("readForm(): should read and return a 'quote' form", () => {
    ...
    assertEquals(result, createListNode([
...
test("readForm(): should read and return a Vec for '['", () => {
    ...
    const result = readForm(rdr);
    assertEquals(result, createVectorNode([
        createSymbolNode('symbol1'),
        createSymbolNode('symbol2'),
    ]));
});
...
test('readSequence(): should parse a vector with one element', () => {
    const rdr = new Reader(['[', '1', ']']);
    assertEquals(readSequence(rdr, ']'), createVectorNode([createNumberNode(1)]));
});
test('readSequence(): should parse a vector with multiple elements', () => {
    const rdr = new Reader(['[', '1', '2', '3', ']']);
    assertEquals(readSequence(rdr, ']'), createVectorNode([
        createNumberNode(1),
        createNumberNode(2),
        createNumberNode(3),
    ]));
...
test('readSequence(): should parse a map with one key-value pair', () => {
    ...
    assertEquals(readSequence(rdr, '}'), createMapNode(new Map([['key:', createNumberNode(1)]])));
});
test('readSequence(): should parse a map with multiple key-value pairs', () => {
    const rdr = new Reader(['{', 'key1:', '1', 'key2:', '2', '}']);
    assertEquals(readSequence(rdr, '}'), createMapNode(new Map([
...
test('readString(): basic list', () => {
    assertEquals(readString('(+ 1 2)'), createListNode([
        createSymbolNode('+'),
        createNumberNode(1),
        createNumberNode(2),
    ]));
...
test('readString(): nested lists', () => {
    assertEquals(readString('((3 4))'), createListNode([
        createListNode([
            createNumberNode(3),
            createNumberNode(4),
        ]),
...
test('readString(): Nested lists with symbols', () => {
    assertEquals(readString('(+ 1 (+ 2 3))'), createListNode([
        createSymbolNode('+'),
        createNumberNode(1),
        createListNode([
            createSymbolNode('+'),
...
test('readString(): List with a negative number', () => {
    assertEquals(readString('(* -3 6)'), createListNode([
        createSymbolNode('*'),
        createNumberNode(-3),
        createNumberNode(6),
    ]));
...
test('readString(): double-quoted string with parens', () => {
    assertEquals(readString('"abc (with parens)"'), createStringNode('abc (with parens)'));
});
...
test('readString(): read of ^/metadata', () => {
    assertEquals(readString('^{"a" 1} [1 2 3]'), createListNode([
        createSymbolNode('with-meta'),
        createVectorNode([
            createNumberNode(1),
            createNumberNode(2),
...
test('readString(): quote special form', () => {
    assertEquals(readString("'1"), createListNode([
        createSymbolNode('quote'),
        createNumberNode(1),
    ]));
});
...
test('readString(): quasiquote special form with unquote', () => {
    assertEquals(readString('`(1 ~a 3)'), createListNode([
        createSymbolNode('quasiquote'),
        createListNode([
            createNumberNode(1),
            createListNode([
...
test('readString(): keywords in list', () => {
    assertEquals(readString('(kw1: kw2: kw3:)'), createListNode([
...
test('readString(): vector with symbol', () => {
    assertEquals(readString('[+ 1 2]'), createVectorNode([
        createSymbolNode('+'),
        createNumberNode(1),
        createNumberNode(2),
    ]));
...
test('readString(): nested vector', () => {
    assertEquals(readString('[[3 4]]'), createVectorNode([
        createVectorNode([
            createNumberNode(3),
            createNumberNode(4),
        ]),
...
test('readString(): nested vector with symbols', () => {
    assertEquals(readString('[+ 1 [+ 2 3]]'), createVectorNode([
        createSymbolNode('+'),
        createNumberNode(1),
        createVectorNode([
            createSymbolNode('+'),
...
test('readString(): map with string key', () => {
    assertEquals(readString('{"abc" 1}'), createMapNode(new Map([
        ['"abc"', createNumberNode(1)],
    ])));
});
test('readString(): nested map', () => {
    assertEquals(readString('{"a" {"b" 2}}'), createMapNode(new Map([
        [
            '"a"',
            createMapNode(new Map([
...
test('readString(): deeply nested map', () => {
    assertEquals(readString('{"a" {"b" {"c" 3}}}'), createMapNode(new Map([
        [
            '"a"',
            createMapNode(new Map([
                [
...
test('readString(): deeply nested map with whitespace', () => {
    assertEquals(readString('{  "a"  {"b"   {  "cde"     3   }  }}'), createMapNode(new Map([
        [
            '"a"',
            createMapNode(new Map([
                [
...
test('readString(): a normal map', () => {
    assertEquals(readString('{"a1" 1 "a2" 2 "a3" 3}'), createMapNode(new Map([
...
test('readString(): map with potentially tricky key/value pair', () => {
    assertEquals(readString('{"1" 1}'), createMapNode(new Map([
...
test('readString(): Testing read of @/deref', () => {
    assertEquals(readString('@a'), createListNode([
        createSymbolNode('deref'),
        createSymbolNode('a'),
    ]));
});
